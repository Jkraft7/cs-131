...
bool Book::operator==(const Book& rhs) const noexcept {
  ...
  if(isbn_ == rhs.isbn_ && author_ == rhs.author_ && title_ == rhs.title_ && price_ == rhs.price_)
    return true;
  else
   return false; 
}
...
bool Book::operator<(const Book& rhs) const noexcept {
  ...
  if(isbn_ < rhs.isbn_)
    return true;
  else if(author_ < rhs.author_)
    return true;
  else if(title_ < rhs.title_)
    return true;
  else if(price_ < rhs.price_)
    return true;
  else 
    return false;
}
...
std::istream & operator>>(std::istream& stream, Book& book) {
  ...
  stream >> std::quoted(book.title_) >> delim; 
  stream >> std::quoted(book.author_) >> delim;
  stream >> book.price_;
  return stream;
